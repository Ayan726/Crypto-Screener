...
const SaveBtn = ({data}) => {
  ...
  return (
    <button
    onClick={handleClick}
    className='border-0 outline-0 bg-none ml-3 mr-2'>
                        <svg className={`w-[1.5rem] ${allCoins.includes(data.id) ? 'fill-cyan' : 'fill-gray-100'}  hover:fill-cyan`} width="30" height="30" viewBox="0 0 30 30"  xmlns="http://www.w3.org/2000/svg">
...
function TableComponent() {
  ...
  return (
    ...
    <div className='w-full h-full rounded flex flex-col relative border-[1.5px] border-gray-100 mt-9'>

      {
        cryptoData ? <table className='w-full table-auto'>
          <thead className='capitalize border-b-[1.5px] border-gray-100 text-base text-gray-100 font-medium'>
            <tr>
          ...
          <tbody className='capitalize'>
            {
              cryptoData.map(data => {
                return (
                  <tr key={data.id} className='text-center text-base border-b-[1.5px] border-gray-100 hover:bg-gray-200 last:border-b-0'>
                    ...
                    <td className={`py-4 ${data.market_cap_change_percentage_24h>0 ? 'text-green md:table-cell hidden' : 'text-red md:table-cell hidden'}`}>{Number(data.market_cap_change_percentage_24h
).toFixed(4)}%</td>
                    <td className={`py-4 ${data.price_change_percentage_1h_in_currency>0 ? 'text-green lg:table-cell hidden' : 'text-red lg:table-cell hidden'}`}>{Number(data.price_change_percentage_1h_in_currency).toFixed(2)
}</td>
                    <td className={`py-4 ${data.price_change_percentage_24h_in_currency>0 ? 'text-green lg:table-cell hidden' : 'text-red lg:table-cell hidden'}`}>{Number(data.price_change_percentage_24h_in_currency).toFixed(2)
}</td>
                    <td className={`py-4 ${data.price_change_percentage_7d_in_currency>0 ? 'text-green lg:table-cell hidden' : 'text-red lg:table-cell hidden'}`}>{Number(data.price_change_percentage_7d_in_currency).toFixed(2)
}</td>
    ...
    <div className='flex sm:flex-row flex-col sm:items-center items-start justify-between mt-4 h-[4rem] gap-y-4'>
    <span className='capitalize'>Data provided by <a className='text-cyan' target='_blank' href="https://www.coingecko.com/">CoinGecko</a></span>
    <Pagination/>
    </div>
