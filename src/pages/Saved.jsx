...
function Saved() {
  ...
  return (
    <section className='w-[80%] h-full flex flex-col mt-16 mb-24 relative'>
      ...
      <div className='w-full min-h-[60vh] rounded relative border-[1.5px] border-gray-100'>
        ...
        <table className='w-full table-auto'>
          <thead className='capitalize border-b-[1.5px] border-gray-100 text-base text-gray-100 font-medium'>
            <tr>
              <th className='py-1 w-[50%] sm:w-auto'>asset</th>
          ...
          <tbody className='capitalize'>
            {
              savedData.map(data => {
                return (
                  <tr key={data.id} className='text-center text-base border-b-[1.5px] border-gray-100 hover:bg-gray-200'>
                    ...
                    <td className={`py-4 md:table-cell hidden ${data.market_cap_change_percentage_24h>0 ? 'text-green' : 'text-red'}`}>{Number(data.market_cap_change_percentage_24h
).toFixed(4)}%</td>
                    <td className={`py-4 lg:table-cell hidden ${data.price_change_percentage_1h_in_currency>0 ? 'text-green' : 'text-red'}`}>{Number(data.price_change_percentage_1h_in_currency).toFixed(2)
}</td>
                    <td className={`py-4 lg:table-cell hidden ${data.price_change_percentage_24h_in_currency>0 ? 'text-green' : 'text-red'}`}>{Number(data.price_change_percentage_24h_in_currency).toFixed(2)
}</td>
                    <td className={`py-4 lg:table-cell hidden ${data.price_change_percentage_7d_in_currency>0 ? 'text-green' : 'text-red'}`}>{Number(data.price_change_percentage_7d_in_currency).toFixed(2)
}</td>
        ...
        :
        <h1 className='min-h-[60vh] text-lg text-cyan flex justify-center items-center'>There's no data to display!</h1>
        }

        <button onClick={resetSavedResult} className='outline-0 border-none bg-none cursor-pointer hover:scale-110 transition-all absolute -top-12 right-0'>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="1em"
            height="1em"
            className='w-7 h-full fill-cyan'
            style={{
